...
//        dd($frontcategories);
        ...
        $sliders = Banner::where(['status' => 1, 'category_id' => 1])
            ...
            $adBanner= Banner::where(['status'=>1, 'category_id'=> 8])->select('id', 'image', 'link')->first();
        ...
        $homeproducts = Category::where(['front_view' => 1, 'status' => 1])
         ...
         $featureproducts = Product::where(['status'=>1, 'feature_product'=>1])->get();
        // return $homeproducts;
        return view('frontEnd.layouts.pages.index', compact('featureproducts','sliders', 'frontcategories', 'hotdeal_top', 'hotdeal_bottom', 'homeproducts', 'sliderbottomads', 'footertopads','newCategories','featuredCategories','offerProducts','adBanner'));
    }
...
//        dd($products);
    ...
    {
        $featureproducts = Product::where(['status'=>1, 'feature_product'=>1])
        ->select('id', 'name', 'slug', 'new_price', 'old_price')
        ->paginate(36);
        return view('frontEnd.layouts.pages.feature-products', compact('featureproducts'));
    }
